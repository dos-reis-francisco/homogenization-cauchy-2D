...
% 25.12.2023 modified for lattice with non cartesian basis

function [ME1,MS4,MatVo,rho]=homogenization(Tb,L1,L2,Y1,Y2,nbeams,nnodes,nodes,Ob,Eb,...
    delta1,delta2,Elast)
    %% inputs :
    ...
    P1=[L1*Y1(1) L1*Y1(2)]; 
    P2=[L2*Y2(1) L2*Y2(2)];
    % coeffs of [du1/dx du1/dy du2/dx du2/dy]
    ...
    for i=1:nbeams 
        ...
        % function to evaluate length and director of the beam
        [Lb(i),e(i,:)]=EvaluateLb2(i,indexn1,indexn2,nodes,L1,L2,delta1,delta2,Y1,Y2);              %*
        ...
        kbL=-kf(i)*Lb(i)/2;   % modulus of rotations in shear force 
    ...
    %% Solve [X] as [K][X]=[B]
    % [K] is singular because of translation possibility of entire lattice
    % we assume dx1=0 dy1=0

    X(3:3*nnodes,:)=K(3:3*nnodes,3:3*nnodes)\B(3:3*nnodes,:);

    % Remove rotational rigidity
    for i=3:3:3*nnodes   
    ...
    %% Evaluation of stiffness tensor taking into account the change in the coordinate system
    g=[P1;P2];
    detg=det(g);
    ...
    % MS4 compliance tensor in vectorial form [S11, S22, S66, S26, S16, S12]
